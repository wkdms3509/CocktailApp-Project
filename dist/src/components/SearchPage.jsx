...
const SearchPage = () => {
    ...
    return (<div className="x-full mx-auto text-center my-10 p-5 lg:px-32">
      <div className="w-3/5 mx-auto flex justify-center pt-5 pb-3 mb-5 gap-x-3 items-center border-b-2 border-black">
        <BsSearch size={22}/>
        <input type="text" placeholder="ex. martini" autoFocus autoComplete="off" value={search} onChange={handleSearchValue} className="flex-1 outline-none border-none p-2"/>
      ...
      {search && filteredItem.length > 0 ? (<>
          <div className="overflow-x-auto mx-auto mt-20 justify-around gap-x-2 flex flex-wrap lg:gap-x-0">
            {filteredItem.map((item) => (<div key={item.id} className="flex md:flex-auto px-2 pb-12 lg:w-1/3 bg-blue-600">
                <Link href={`/products/${item.id}`}>
                  <div key={item.id} className="text-left">
                    <Image src={item.img} alt={item.name} width="400" height="300" 
            // className="ProductCardImg"
            className="scale-100 object-cover hover:scale-105 duration-300 w-80" priority/>
