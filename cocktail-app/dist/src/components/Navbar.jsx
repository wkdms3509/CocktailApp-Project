...
const Navbar = () => {
    ...
    return (<>
      {status === "authenticated" ? (<header>
          ...
          <div className={classNames("app_banner_top", appToggle ? "app_banner_top_close" : "app_banner_top")}>
            <ul className="flex justify-between items-center align-middle">
              ...
              <li className="items-center">
                <IoCloseOutline size={30} className="items-center font-extralight text-gray-300 cursor-pointer" onClick={() => handleCloseAppToggle()}/>
              </li>
            </ul>
          </div>
          <nav className="nav z-20">
            <div className="hidden lg:block">
              <ul className="side_category">
                {(session === null || session === void 0 ? void 0 : session.user) && session.user.auth === "admin" ? (<li className="side_category_menu text-blue-700">
                    {session.user.auth}
                  </li>) : (<li className="side_category_menu font-semibold">
                    {(_a = session.user) === null || _a === void 0 ? void 0 : _a.name} 님
                  </li>)}
            ...
            <div className="main_category">
              <div className="flex items-center justify-between z-30">
                ...
                <div className="lg:hidden flex items-center">
                  ...
                  {/* </li> */}
                  {toggle ? (<AiOutlineClose size={25} onClick={handleSideBtnClick} className="cursor-pointer hover:text-gray-400"/>) : (<AiOutlineMenu size={25} onClick={handleSideBtnClick} className="cursor-pointer hover:text-gray-400"/>)}
                </div>
              </div>
              {/* 반응형 */}
              <ul className={classNames("main_category_right_one", {
                ["opacity-100 top-26"]: toggle,
            })}>
                <Link href="/">
                  <li className="category_right_menu font-bold" onClick={handleClickMenu}>
                    HOME
                  </li>
                </Link>
                <Link href="/products/recommend">
                  <li className="category_right_menu font-normal" onClick={handleClickMenu}>
                    RECOMMAND
                  </li>
                </Link>
                {((_b = session === null || session === void 0 ? void 0 : session.user) === null || _b === void 0 ? void 0 : _b.auth) === "admin" ? (<Link href="/products/new">
                    <li className="category_right_menu font-normal" onClick={handleClickMenu}>
                      NEW
                    </li>
                  </Link>) : ("")}
                {status === "authenticated" ? (<Link href="/auth/mypage">
                    <li className="category_right_menu font-normal" onClick={handleClickMenu}>
                      MY
                    </li>
                  </Link>) : ("")}
                ...
                {session ? (<li className="category_right_menu lg:hidden" onClick={() => signOut({ callbackUrl: "/" })}>
                    LOGOUT
                  </li>) : ("")}
              </ul>
            </div>
            <div>
              ...
              <ul className="sub_category gap-x-3">
                ...
                <Link href={`/category/${type.gin}`} className="nav_category_menu_link">
                  <li className="sub_category_menu one active text-sm md:text-base lg:text-base xl:text-base">
                    <span>GIN</span>
                    <span className="absolute -bottom-1 left-0 w-0 transition-all h-1 bg-blue-500"></span>
                  </li>
                </Link>
                <Link href={`/category/${type.non_alc}`} className="nav_category_menu_link">
                  <li className="sub_category_menu one active text-sm md:text-base lg:text-base xl:text-base">
                    <span>NON-ALCOL</span>
                    <span className="absolute -bottom-1 left-0 w-0 transition-all h-1 bg-blue-500"></span>
                  </li>
                </Link>
                <Link href={`/category/${type.vodka}`} className="nav_category_menu_link">
                  <li className="sub_category_menu one text-sm md:text-base lg:text-base xl:text-base">
                    <span>VODKA</span>
                    <span className="absolute -bottom-1 left-0 w-0 transition-all h-1 bg-blue-500"></span>
                  </li>
                </Link>
                <Link href={`/category/${type.rum}`} className="nav_category_menu_link">
                  <li className="sub_category_menu one text-sm md:text-base lg:text-base xl:text-base">
                    <span>RUM</span>
                    <span className="absolute -bottom-1 left-0 w-0 transition-all h-1 bg-blue-500"></span>
                  </li>
